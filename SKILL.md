...
| `Target page, context or browser has been closed` | **Profile corrupted** from repeated debug sessions | Delete `.playwright-profile/` and try with fresh profile |
| reCAPTCHA picture verification | Google flagged IP/profile from too many rapid scrapes | **STOP for the day.** Wait 24h. Don't retry - each attempt makes it worse |
...
## Known Issues (Battle-Tested, Feb 2026)

### Profile Poisoning
Repeated rapid scrapes (especially during debugging) corrupt the persistent browser profile. Symptoms: 0 ads, browser crashes, `Target page, context or browser has been closed`. **Fix:** Delete `.playwright-profile/` to reset. Fresh profile works immediately but may trigger one-time CAPTCHA.

### Proxy Is Non-Optional for Ads
UULE alone will NOT get ads when scraping from outside the target country. Google Ads uses the **real IP** for auction eligibility. You MUST use `--proxy` or a VPN. Organic results work without proxy.

### Ephemeral Ads
Google sometimes renders ads briefly then strips them via JS. This is an UNSOLVED issue as of Feb 2026. The kurama-susanoo-serp dashboard has partial response-interception code for this, but it's not proven. The CLI does not have this yet.

### Rate Limits
After ~10+ scrapes in an hour from the same proxy/IP, Google starts returning reCAPTCHA or 0 results. This is cumulative across CLI and dashboard (they share the same `.playwright-profile` and proxy). **Limit to 1-2 scrape sessions per day.**
...
| Deleting `.playwright-profile/` after every scrape | Triggers reCAPTCHA every time | Keep persistent profile, only delete when corrupted |
| Scraping every hour | Unnecessary, ads don't change that fast | Daily or weekly is sufficient |
| Rapid debug cycles (10+ scrapes/hour) | Burns proxy IP and poisons profile | Test deliberately, wait between scrapes |
| Retrying after reCAPTCHA | Makes Google MORE suspicious | Stop for the day, wait 24h |
